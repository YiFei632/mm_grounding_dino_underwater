...
class PackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # Preserve explicit entity metadata generated by datasets with
        # ``return_classes=True`` so grounding models can use the class list
        # directly instead of recovering it through NER.
        if 'custom_entities' in results:
            data_sample.custom_entities = results['custom_entities']
        if 'caption_prompt' in results:
            data_sample.caption_prompt = results['caption_prompt']
