...
class ThirdGroundingDinoTransformerDecoderLayer(
...
class ThirdGroundingDinoTransformerEncoder(DeformableDetrTransformerEncoder):
...
class ThirdGroundingDinoTransformerDecoder(DinoTransformerDecoder):

    def _init_layers(self) -> None:
        """Initialize decoder layers."""
        self.layers = ModuleList([
            ThirdGroundingDinoTransformerDecoderLayer(**self.layer_cfg)
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims
