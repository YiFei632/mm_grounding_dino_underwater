...
class SonarBindPrismPretrain(ThirdGroundingDINO):
    ...
    def __init__(self,
        ...
        if self.pretrain_mode:
            self._configure_single_modality_pretrain()

    def _configure_single_modality_pretrain(self) -> None:
        """Freeze detector and the non-active modality for specialist stages."""
        inactive = self.sonar_backbone if self.pretrain_modality == 'rgb' else self.backbone
        for parameter in inactive.parameters():
            parameter.requires_grad = False
        for module_name in ('decoder', 'bbox_head'):
            for parameter in getattr(self, module_name).parameters():
                parameter.requires_grad = False
        for name in ('fusion_image_sonar_layers', 'fusion_visual_text_layers'):
    ...
    def extract_sonar_roi_features(self, batch_inputs, gt_bboxes):
        """Extract RoI features from the dedicated sonar backbone.

        In sonar-specialist pretraining ``batch_inputs`` is the sonar image
        loaded by the dataset itself and the boxes come from that sonar
        dataset's COCO annotations.  Boxes are mapped explicitly from the
        padded input coordinates to the actual backbone feature coordinates,
        avoiding the RGB neck's fixed 1/8 spatial scale.
        """
        ...
        sonar_feat = sonar_feats[-1] if isinstance(
            sonar_feats, (tuple, list)) else sonar_feats
        batch_size, channels, feat_h, feat_w = sonar_feat.shape
        mapped = self.sonar_feat_map(
            sonar_feat.flatten(2).transpose(1, 2))
        mapped = mapped.transpose(1, 2).reshape(
            batch_size, self.embed_dims, feat_h, feat_w)

        input_h, input_w = batch_inputs.shape[-2:]
        scale_x = feat_w / float(input_w)
        scale_y = feat_h / float(input_h)
        roi_list = []
        for batch_idx, boxes in enumerate(gt_bboxes):
            if len(boxes) == 0:
                continue
            scaled_boxes = boxes.clone()
            scaled_boxes[:, 0::2] *= scale_x
            scaled_boxes[:, 1::2] *= scale_y
            batch_indices = scaled_boxes.new_full((len(boxes), 1), batch_idx)
            roi_list.append(torch.cat([batch_indices, scaled_boxes], dim=1))

        if not roi_list:
            return mapped.new_empty((0, self.embed_dims))

        from torchvision.ops import roi_align
        roi_feats = roi_align(
            mapped,
            torch.cat(roi_list, dim=0),
            output_size=self.pretrain_roi_output_size,
            spatial_scale=1.0,
            sampling_ratio=2,
            aligned=True)
        return roi_feats.mean(dim=(2, 3))
    ...
    def _loss_pretrain(self, batch_inputs, batch_data_samples):
        ...
        if self.pretrain_modality == 'rgb':
            # RGB-specialist stage: train only the RGB feature extraction path
            # and the text path.  Do not execute RGB-sonar/text fusion.
            if self.use_autocast:
                with autocast(enabled=True):
            ...
            # Bypass ThirdGroundingDINO/SonarBind pre_transformer extensions:
            # specialist RGB datasets do not provide a paired sonar image.
            encoder_inputs_dict, _ = GroundingDINO.pre_transformer(
                self, visual_features, batch_data_samples)
            ...
            roi_feats = self.extract_roi_features(
                visual_feats, gt_bboxes,
                encoder_inputs_dict['spatial_shapes'])
        elif self.pretrain_modality == 'sonar':
            # Sonar-specialist datasets load the sonar frame as batch_inputs and
            # supply boxes from their own COCO annotation file.
            roi_feats = self.extract_sonar_roi_features(
                batch_inputs, gt_bboxes)
        else:
            raise ValueError(
                'pretrain_modality must be either "rgb" or "sonar", '
                f'but got {self.pretrain_modality!r}')
