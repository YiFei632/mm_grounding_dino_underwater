...
class SonarBindPrism(ThirdGroundingDINO):
    ...
    def _configure_single_modality_pretrain(self) -> None:
        """Freeze the branch and fusion modules not used by pretraining.

        Single-modality datasets should not update the cross-modal fusion or
        detector.  The active backbone, projection/neck, and text encoder
        remain trainable for the ROI-text pretraining objective.
        """
    ...
    def pre_decoder(
        ...
        decoder_inputs_dict = dict(
            ...
            memory=memory,
            memory_sonar=memory_sonar,
            sonar_attention_mask=memory_sonar_mask,
