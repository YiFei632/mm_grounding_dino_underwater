...
pretrained = '/media/fishyu/fish-14tb-2/YiFei/Grounding_DINO/mmdetection/checkpoints/swin_large_patch4_window12_384_22k.pth'  # noqa
...
model = dict(
    ...
    sonar_backbone=dict(                                                                                                    
        ...
        convert_weights=True,
        frozen_stages=-1,                                                                                                   
        init_cfg=dict(type='Pretrained', checkpoint=pretrained)  # 使用与图像backbone相同的预训练权重                       
    ),
    neck=dict(
        type='ChannelMapper',
