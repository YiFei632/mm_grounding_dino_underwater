...
model = dict(
    ...
    language_model=dict(
        ...
        # GLIP-aligned configurations
        use_checkpoint=True,  # Enable gradient checkpointing to save memory
        num_layers_of_embedded=-1,  # Use all BERT layers (GLIP default)
    ),
    backbone=dict(
        type='ResNet',
        depth=50,
    ...
    encoder=dict(
        ...
        # text layer config (process projected 256-dim text features)
        text_layer_cfg=dict(
            self_attn_cfg=dict(num_heads=4, embed_dims=256, dropout=0.0),
            ffn_cfg=dict(
                embed_dims=256, feedforward_channels=1024, ffn_drop=0.0)),
        # fusion layer config for image-sonar (both are 256-dim)
        fusion_image_sonar_cfg=dict(
            v_dim=256,
            l_dim=256,  # Sonar features are 256-dim
            embed_dim=256,
            num_heads=8,
            init_values=1e-4),
        # fusion layer config for visual-text (text features are projected to 256-dim)
        fusion_layer_cfg=dict(
            v_dim=256,
            l_dim=256,  # Text features are projected to 256-dim by text_feat_map
            embed_dim=256,
            num_heads=8,
            init_values=1e-4),
    ),
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'language_model': dict(lr_mult=0.01),  # GLIP uses lower LR for language model
            'sonar_backbone': dict(lr_mult=0.1),
        }))

# learning policy
max_epochs = 6  # 先行进行测试，训练两轮之后微调看效果
param_scheduler = [
