...
model = dict(
    ...
    sonar_backbone=dict(
        type='LGANet',
        out_indices=(3,),   # P5/32, 512ch
        frozen_stages=-1,   # 不冻结（无预训练权重时从头训练）
        norm_eval=False,
        init_cfg=dict(type='Pretrained', checkpoint='/media/fishyu/fish-14tb-2/YiFei/Grounding_DINO/mmdetection/checkpoints/lganet_pretrained_rgbs50.pth')       # 有预训练权重后改为 dict(type='Pretrained', checkpoint='...')
    ),
    sonar_feat_channels=512,  # LGANet P5 输出通道数（ResNet-50 为 2048）
