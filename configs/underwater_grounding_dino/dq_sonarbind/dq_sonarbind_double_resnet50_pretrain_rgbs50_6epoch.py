...
model = dict(
    type='DQSonarBind',
    num_queries=900,  # 与 dynamic_query_list 的最大值保持一致，用于 query_embedding 的大小
    with_box_refine=True,
    as_two_stage=True,
    # --- DQ-DETR 新增字段 ---
    ccm_cfg=dict(cls_num=4),
    cgfe_cfg=dict(gate_channels=256, num_feature_levels=4),  # 4尺度（非5尺度）
    dynamic_query_list=[300, 500, 700, 900],  # 对应 cls_num=4 的四个密度等级
    # --- 以下与 SonarBind baseline 完全一致 ---
