...
model = dict(
    ...
    sonar_backbone=dict(
        ...
        frozen_stages=4,  # 全部冻结（预训练不用sonar）
    ...
    decoder=dict(
        ...
        layer_cfg=dict(
            ...
            ffn_cfg=dict(
                embed_dims=256, feedforward_channels=2048, ffn_drop=0.0),
            use_text_cross_attn=True),  # 预训练模式需要文本交叉注意力
        post_norm_cfg=None),
    positional_encoding=dict(
        num_feats=128, normalize=True, offset=0.0, temperature=20),
    bbox_head=dict(
        type='GroundingDINOHead',
        num_classes=3,  # UTDAC2020有4个类别
...
# dataset settings
# UTDAC2020数据集根目录
data_root = '/media/fishyu/fish-14tb-2/YiFei/Dataset/UXO_COCO/'
...
# UTDAC2020数据集类别定义
metainfo = {
    'classes': ('100lbs_aircraft_bomb', '15cm_mortar', '20lbs_incendiary'),
...
train_dataloader = dict(
    _delete_=True,
    batch_size=6,  # 预训练可以用较小的batch size
    num_workers=4,
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'backbone': dict(lr_mult=0.2),  # RGB Backbone用小学习率
            'language_model': dict(lr_mult=0.1),
            'sonar_backbone': dict(lr_mult=0.0),  # Sonar完全冻结
...
train_cfg = dict(
    type='EpochBasedTrainLoop', max_epochs=max_epochs, val_interval=2)  # 每2个epoch验证一次
...
load_from = '/media/fishyu/fish-14tb-2/YiFei/Grounding_DINO/mmdetection/work_dirs/sonarbind_prism_double_resnet50_pretrain_utdac2020/epoch_12.pth'