...
pretrained = '/media/fishyu/fish-14tb-2/YiFei/Grounding_DINO/mmdetection/checkpoints/swin_large_patch4_window12_384_22k.pth'
...
model = dict(
    ...
    ccm_cfg=dict(cls_num=4),
    cgfe_cfg=dict(gate_channels=256, num_feature_levels=5),
    dynamic_query_list=[300, 500, 900, 1200],
...
train_dataloader = dict(
    batch_size=1,  # 减小到1（禁用 gradient checkpointing 后需要）
    num_workers=8,
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type='AdamW',
        lr=0.0005,  # 0.0002 for DeformDETR
        weight_decay=0.0001),
...
optim_wrapper.update(dict(
    accumulative_counts=4  # 增加到4以保持有效 batch_size=32
))
...
model_wrapper_cfg = dict(
    ...
    # 移除 static_graph 以避免与 AMP 冲突
    broadcast_buffers=False,
)

default_hooks = dict(
    timer=dict(type='IterTimerHook'),
    logger=dict(type='LoggerHook', interval=50),  # 每50个iter打印一次日志
